...
__export(index_exports, {
  CANCELED_STATUS_CODES: () => CANCELED_STATUS_CODES,
  DELIVERED_STATUS_CODES: () => DELIVERED_STATUS_CODES,
  NOVAPOSHTA_MODULE: () => NOVAPOSHTA_MODULE,
  NP_TRACKING_URL: () => NP_TRACKING_URL,
  NovaPoshtaClient: () => NovaPoshtaClient,
  NovaPoshtaError: () => NovaPoshtaError,
  NovaPoshtaErrorCode: () => NovaPoshtaErrorCode,
  NovaPoshtaService: () => service_default,
  citySearchQuerySchema: () => citySearchQuerySchema,
  fulfillmentDataSchema: () => fulfillmentDataSchema,
  isRetryableError: () => isRetryableError,
  novaPoshtaOptionsSchema: () => novaPoshtaOptionsSchema,
  parseOrThrow: () => parseOrThrow,
  trackingStatusSchema: () => trackingStatusSchema,
  warehouseQuerySchema: () => warehouseQuerySchema,
  webhookPayloadSchema: () => webhookPayloadSchema,
  wrapApiError: () => wrapApiError
});
module.exports = __toCommonJS(index_exports);

// src/modules/novaposhta/service.ts
var import_utils = require("@medusajs/framework/utils");
...
var novaPoshtaOptionsSchema = import_zod.z.object({
  apiKey: import_zod.z.string().min(1, "apiKey is required"),
  senderRef: import_zod.z.string().min(1, "senderRef is required"),
  senderCityRef: import_zod.z.string().min(1, "senderCityRef is required"),
  contactRef: import_zod.z.string().min(1, "contactRef is required"),
  phone: import_zod.z.string().min(10, "phone must be at least 10 characters"),
  webhookSecret: import_zod.z.string().optional(),
  cacheTtlMs: import_zod.z.number().positive().optional()
});
var fulfillmentDataSchema = import_zod.z.object({
  city_ref: import_zod.z.string().min(1, "city_ref is required"),
  warehouse_ref: import_zod.z.string().min(1, "warehouse_ref is required"),
  service_type: import_zod.z.enum(["novaposhta_warehouse", "novaposhta_courier"]).optional().default("novaposhta_warehouse")
});
var citySearchQuerySchema = import_zod.z.object({
  city: import_zod.z.string().optional().default("")
});
var warehouseQuerySchema = import_zod.z.object({
  city_ref: import_zod.z.string().min(1, "city_ref is required")
});
var webhookPayloadSchema = import_zod.z.object({
  ttn: import_zod.z.string().min(1),
  status: import_zod.z.string().min(1),
  status_code: import_zod.z.union([import_zod.z.string(), import_zod.z.number()]).optional(),
  fulfillment_id: import_zod.z.string().optional(),
  metadata: import_zod.z.record(import_zod.z.unknown()).optional()
});
var trackingStatusSchema = import_zod.z.object({
  Number: import_zod.z.string().optional(),
  Status: import_zod.z.string().optional(),
  StatusCode: import_zod.z.union([import_zod.z.string(), import_zod.z.number()]).optional(),
  WarehouseRecipient: import_zod.z.string().optional(),
  ScheduledDeliveryDate: import_zod.z.string().optional()
});
...
var NovaPoshtaService = class extends import_utils.AbstractFulfillmentProviderService {
